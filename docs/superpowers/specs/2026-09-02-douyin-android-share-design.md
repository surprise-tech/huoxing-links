...
第一阶段只交付一个可真实验收的闭环：用户在独立 Android 应用“极风小助手”中点击“分享到抖音好友/群”，抖音展示联系人或群选择页；用户手动选择并发送后，接收方看到链接卡片，点击卡片后经过现有安全跳转链路，最终看到指定的微信二维码。
...
- 添加一个测试抖音号白名单。
- 创建一个永久、单二维码、无过期、无 UV 上限的验收链接，并提交精确分享 URL 审核。
...
```text
抖音私信卡片
  -> approvedShareUrl（由 PUBLIC_ORIGIN 与不可变的 8 位 LINK_CODE 组成）
  -> HTTP 302 /j/{LINK_CODE}
  -> GET /api/link-target/{LINK_CODE}
  -> /qr/{LINK_CODE}?visitor_token={短期令牌}
  -> GET /api/link-show-qr/{LINK_CODE}
  -> 展示固定微信二维码
```

这些层分别承担 canonical 地址、跳转页面、目标解析、短期选择令牌和二维码渲染，属于正常的业务分层，不额外叠加伪装页。

验收卡片在审核和测试期间固定为一个二维码，不启用二维码轮换，不修改最终目标。若该 URL 被抖音拒绝，不通过增加中间域名绕过；改用公司主体可控且符合平台要求的域名重新部署并重新审核。
...
6. 在现有后台创建永久单码微信二维码链接，确认无过期和 UV 限制。
7. 对 canonical 分享 URL 做裸网络与手机浏览器验收。
...
- 后端跳转目标只允许现有协议和域名策略；Android 不直接解析或执行二维码最终目标。
- 固定验收链接不可因会员到期、链接到期或 UV 限额失效。
- 二维码页只展示用户提供并授权使用的二维码，不自动发送消息。
...
- 服务端相关测试：固定链接永久有效、单码无上限、二维码页可读取且不会重复计量。
...
7. 接收方看到链接卡片。
8. 点击卡片后依次进入审核 URL、内部解析和二维码页。
9. 微信二维码清晰可识别，重复打开仍有效。
