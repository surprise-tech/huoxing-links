...
const rules = {
  ...
  'config.url': [{ required: true, message: '落地小程序页面路径不能为空', trigger: 'blur' }]
}
...
const selectMin = (value: any) => {
  miniProgramList.value.forEach((item: any) => {
    if (item.id == value) {
      formData.value.config.url = item.url
    }
  })
}
...
<template>
  ...
  <div class="main">
    ...
    >
      ...
      <el-form-item prop="config.min_id" label="落地小程序" required>
        <el-select v-model="formData.config.min_id" placeholder="请选择" @change="selectMin">
          <el-option v-for="item in miniProgramList" :key="item.id" :label="item.name" :value="item.id">
            <span>{{ item.name }}</span>
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item prop="config.url" label="落地小程序页面路径">
        <el-input v-model="formData.config.url" placeholder="例如：/pages/index"></el-input>
      </el-form-item>
    </el-form>
